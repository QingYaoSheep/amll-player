...
最新增量以 `Remaining-Visual-Port.md` 的 2026-09-20 记录为准：词来源映射、共享回放、分组强调及 PNG 差异工具已补充；工具通过与设备效果通过分开记录。全量跨端视觉及性能仍未签收，下方历史缺项不是当前完成状态表。
...
最新补充：[剩余视觉能力实施台账](Remaining-Visual-Port.md)。历史“只有 Debug 歌词区”结论不再代表当前真实页面接线状态；剩余 HDR/Pixi/动态封面等不得因基础模型存在而勾选完成。
